...
/**
 * Simple list fragment which observe pull requests from local cache.
 */
class PullRequestListFragment : BaseListFragment(){
    ...
    override fun initAdapterAndUI(list: RecyclerView, emptyList: TextView, progressBar: ProgressBar) {
        ...
        activity?.let { it ->
            viewModel.pullRequests.observe(it, Observer<PagedList<Issue>> {
